...
@Slf4j
@Component("GetFirstUserData")
@RequiredArgsConstructor // lombok annotation to generate a constructor with required args
public class GetFirstUserDataCommand implements Command {

    private GetFirstUserDataCommand proxy;
    ...
    public void init(Command command) {
        this.proxy = (GetFirstUserDataCommand) command;
    }

    @Trace
    @Override
    public String sender(TraceInfo traceInfo) {
        log.info("SENDER/GetFirstUserData:::{}", traceInfo);
        GetUserDataRequest response = GetUserDataRequest.builder().request("GetFirstUserData").build();
